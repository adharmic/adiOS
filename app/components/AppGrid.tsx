    ...
    <div className="absolute l-0 t-0 z-10 w-fit gap-8 flex flex-col pb-4 flex-wrap max-h-full max-w-full pr-4">
      {possibleApplings.map((app, index) => {
        return (
          <div
            key={index}
            className="hoverGlass appIcon aspect-square flex flex-col w-fit items-center justify-center p-4 lg:h-40 lg:max-h-40 hover:cursor-pointer transition-all active:scale-95 h-32 max-h-32 border border-[rgba(0,0,0,0)]"
            onClick={() => {
